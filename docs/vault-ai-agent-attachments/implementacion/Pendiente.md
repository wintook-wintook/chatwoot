...
- [x] **Punto de intercepción** de `{{nombre}}`: job de envío
...
- [x] Parser de `{{nombre}}` + adjuntar blob al mensaje saliente
...
- [ ] **Enviar `{{nombre}}` también desde la base de conocimiento**
      (`KnowledgeBaseResponseService`): hoy `{{nombre}}` solo se resuelve en la ruta
...
- [x] Subir / listar (scroll, ~3 visibles) / borrar + snippet copiable `{{nombre}}`.
- [x] **Selector de adjuntos** en modal que inserta `{{nombre}}` en el cursor
