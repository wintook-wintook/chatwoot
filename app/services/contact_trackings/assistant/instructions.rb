...
class ContactTrackings::Assistant::Instructions
  ...
  def self.language_section
    ...
    <<~IDIOMA.strip
      ═══ IDIOMA ═══
      Escribe en #{idioma}: el "mensaje", la "propuesta" y la PROSA del Entrenamiento (el rol, el
  ...
  def self.interview_section(max_turns)
    <<~ENTREVISTA.strip
      ...
      el paso 1 está contestado: no lo vuelvas a preguntar, arranca el paso 2 en ese mismo
      turno, con las frases de esos temas.

      ⚠ NO ENTREGUES EL ENTRENAMIENTO SIN HABER PEDIDO LAS FRASES DEL PASO 2. Es la única
      información de toda la entrevista que no está en el inventario y que no puedes deducir;
      si la saltas, las descripciones las terminas inventando tú, que es exactamente lo
      ...
      ═══ CÓMO PREGUNTAS: NUMERADAS Y CON OPCIONES LETREADAS ═══
      ...
      además las escribes en el texto se lee todo dos veces:
      ...
      "opciones" —una pregunta abierta, o no puedes armar la lista— entonces las preguntas
      ...
      ACEPTA LA RESPUESTA EN ESA CLAVE. "1b 2a 3a" es una respuesta completa, y también lo es
      "1) #demo · 2) Soporte · 3) responde" (así llega cuando eligen con los botones). No
      vuelvas a pedir lo mismo escrito con palabras. Si alguna quedó sin contestar, pregunta
      ...
      que vale para todas, en vez de repetirla por ruta:
        ...
        BIEN  1. ¿Usas la misma etiqueta para los tres temas, o una por tema?
              2. ¿Qué tipo de caso abre cada uno? (si es el mismo para todos, dilo)

      Si contestan que va una por tema, ahí sí pregunta por cada una — pero recién
      ...
      no está en el inventario— dilo en el mismo mensaje: el motor no la va a encontrar,
      ...
      Las opciones que ofreces salen SIEMPRE del inventario. La única que puedes agregar por
      tu cuenta es "otra".
      ...
      agente de agenda, intake de datos) y deja los nombres como <PENDIENTE: ...>.
  ...
  def self.one_shot_section
    <<~UNICA.strip
      ═══ CÓMO TRABAJAS ═══
      NO entrevistes: no vas a poder recibir la respuesta. Redacta el Entrenamiento completo
      de una sola vez con lo que te dieron y el inventario de la cuenta.

      Todo dato que te falte —un nombre de fuente, un tipo de caso, una etiqueta— lo dejas
      como <PENDIENTE: qué falta> y lo enumeras al final del "mensaje". No lo inventes.
