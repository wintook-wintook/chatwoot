...
RSpec.describe Tiktok::Client do
  ...
  describe '#image_send_capable?' do
    ...
    it 'returns true when IMAGE_SEND capability is enabled' do
      allow(client).to receive(:process_json_response).with(
        response,
        'Failed to call TikTok business/message/capabilities/get/'
      ).and_return(
        {
          'data' => {
            'capability_infos' => [
      ...
      expect(HTTParty).to have_received(:get).with(
        ...
        headers: { 'Accept' => 'application/json', 'Access-Token' => 'token-123' }
      )
    end
  ...
  describe '#upload_media' do
    ...
    before do
      ...
      allow(blob).to receive(:open) do |&block|
        Rails.root.join('spec/assets/avatar.png').open('rb', &block)
      end
    end
  ...
  describe '#send_media_message' do
    ...
    it 'sends image messages' do
      allow(client).to receive(:upload_media).with('blob').and_return('media-123')
      ...
      expect(client).to have_received(:send_message).with(
        'tt-conv-1', { message_type: 'IMAGE', image: { media_id: 'media-123' } }
      )
    end
  end
end
