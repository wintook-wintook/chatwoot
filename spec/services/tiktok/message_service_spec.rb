...
# Ojo: aquí no se puede usar `Message.last`. Message lleva default_scope order(created_at:
# :asc) y el servicio fija created_at a la hora que manda TikTok, así que cualquier mensaje
# más reciente que haya en la BD ganaría. Se busca siempre por source_id.
RSpec.describe Tiktok::MessageService do
  # Imita lo que devuelve Down.download: un Tempfile con nombre y tipo de contenido.
  def fake_downloaded_image
  ...
  describe '#perform' do
    ...
    it 'creates an incoming image attachment when media is present' do
      ...
      tempfile = fake_downloaded_image
      service = described_class.new(channel: channel, content: content)
      allow(service).to receive(:create_contact_inbox).and_return(contact_inbox)
    ...
    context 'when lock_to_single_conversation is enabled' do
      it 'reuses the last resolved conversation' do
        ...
        expect(resolved_conversation.reload.messages.find_by(source_id: 'tt-msg-1').content).to eq('Hello from TikTok')
      end
    end
    ...
    context 'when lock_to_single_conversation is disabled' do
      ...
      it 'creates a new conversation if the previous one is resolved' do
        ...
        expect(Message.find_by(source_id: 'tt-msg-lock-2').conversation.additional_attributes['conversation_id']).to eq('tt-conv-1')
      end
    end
  end
end
