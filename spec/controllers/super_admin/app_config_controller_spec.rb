  ...
  describe 'POST /super_admin/app_config' do
    ...
    context 'when it is an aunthenticated super admin' do
      ...
      it 'refuses to write facebook credentials through the instagram group' do
        sign_in(super_admin, scope: :super_admin)
        post '/super_admin/app_config?config=instagram', params: { app_config: { FB_APP_SECRET: 'HIJACKED' } }

        expect(GlobalConfig.get('FB_APP_SECRET')['FB_APP_SECRET']).not_to eq('HIJACKED')
      end
    end
  end
end
